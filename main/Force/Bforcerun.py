...
---------------------------------------------------------------
1. Launches *BEXTForce.py* inside Abaqus/CAE (no‑GUI) on the chosen ODB.
...
3. Reads those two numbers, shows them on the terminal, and packs them—
   together with the full stdout/stderr log—into **BForce.json**.
...
CUTFORCE_PY  = r"C:\Users\Ougbine\B\BEXTForce.py"
DEFAULT_ODB  = r"C:\Users\Ougbine\BChipInp.odb"
# ------------------------------------------------------------------ #
...
study    = odb_path.stem               # e.g.  BChip  →  BChip.hrf
...
json_path = pathlib.Path("BForce.json").resolve()
