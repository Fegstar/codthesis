...
---------------------------------------------------------------
1. Launches *TQEXTForce.py* inside Abaqus/CAE (no‑GUI) on the chosen ODB.
...
3. Reads those two numbers, shows them on the terminal, and packs them—
   together with the full stdout/stderr log—into **TQForce.json**.
...
CUTFORCE_PY  = r"C:\Users\Ougbine\TQ\TQEXTForce.py"
DEFAULT_ODB  = r"C:\Users\Ougbine\TQChipInp.odb"
# ------------------------------------------------------------------ #
...
study    = odb_path.stem               # e.g.  TQChip  →  TQChip.hrf
...
# --- assemble and dump TQForce.json -------------------------------- #
record = {
...
json_path = pathlib.Path("TQForce.json").resolve()
