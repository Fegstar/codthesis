...
# ---------------------------------------------------------------------------
mFORCE_JSON = Path(r"C:\Users\Ougbine\mForce.json")
...
def load_forces(json_path: Path) -> tuple[float, float]:
    """Return (force_c, force_p) from mForce.json."""
...
def patch_first_value(path: Path, list_name: str, new_val: float) -> None:
    ...
    for i, line in enumerate(src):
        ...
        if m:
            ...
            elements[4] = f" {new_val:.6f}"  # leading space for readability
...
def main() -> None:
    force_c, force_p = load_forces(mFORCE_JSON)

    print(f"Loaded forces from {mFORCE_JSON}:")
