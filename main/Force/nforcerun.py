...
---------------------------------------------------------------
1. Launches *NEXTForce.py* inside Abaqus/CAE (no‑GUI) on the chosen ODB.
...
3. Reads those two numbers, shows them on the terminal, and packs them—
   together with the full stdout/stderr log—into **NForce.json**.
...
CUTFORCE_PY  = r"C:\Users\Ougbine\N\NEXTForce.py"
DEFAULT_ODB  = r"C:\Users\Ougbine\NChipInp.odb"
# ------------------------------------------------------------------ #
...
study    = odb_path.stem               # e.g.  NChip  →  NChip.hrf
...
json_path = pathlib.Path("NForce.json").resolve()
