...
# ---------------------------------------------------------------------------
BFORCE_JSON = Path(r"C:\Users\Ougbine\BForce.json")
...
def load_forces(json_path: Path) -> tuple[float, float]:
    """Return (force_c, force_p) from BForce.json."""
...
def patch_first_value(path: Path, list_name: str, new_val: float) -> None:
    ...
    for i, line in enumerate(src):
        ...
        if m:
            ...
            elements[2] = f" {new_val:.6f}"  # leading space for readability
...
def main() -> None:
    force_c, force_p = load_forces(BFORCE_JSON)

    print(f"Loaded forces from {BFORCE_JSON}:")
