...
---------------------------------------------------------------
1. Launches *AEXTForce.py* inside Abaqus/CAE (no‑GUI) on the chosen ODB.
...
3. Reads those two numbers, shows them on the terminal, and packs them—
   together with the full stdout/stderr log—into **AForce.json**.
...
CUTFORCE_PY  = r"C:\Users\Ougbine\A\AEXTForce.py"
DEFAULT_ODB  = r"C:\Users\Ougbine\AChipInp.odb"
# ------------------------------------------------------------------ #
...
study    = odb_path.stem               # e.g.  AChip  →  AChip.hrf
...
json_path = pathlib.Path("AForce.json").resolve()
