...
---------------------------------------------------------------
1. Launches *rEXTForce.py* inside Abaqus/CAE (no‑GUI) on the chosen ODB.
...
3. Reads those two numbers, shows them on the terminal, and packs them—
   together with the full stdout/stderr log—into **rForce.json**.
...
CUTFORCE_PY  = r"C:\Users\Ougbine\r\rEXTForce.py"
DEFAULT_ODB  = r"C:\Users\Ougbine\rChipInp.odb"
# ------------------------------------------------------------------ #
...
study    = odb_path.stem               # e.g.  rChip  →  rChip.hrf
...
json_path = pathlib.Path("rForce.json").resolve()
