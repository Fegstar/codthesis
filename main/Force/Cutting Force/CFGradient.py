...
def get_thickness_sensitivities(
    ...
    Cutting_Force,                               # measured / reference thickness
    Pet_Cutting_Force_list,                      # one entry per JC parameter
    eps=0.2                             # relative perturbation size
):
    """
    Returns a list whose first element is `Cutting_Force`, followed by
    ∂h/∂pᵢ for every parameter pᵢ, each evaluated with its own
    `Pet_Cutting_Force_list[i]`.
    """

    # ----- 1) collect parameters in a flat list --------------------
    jc_params = [TQ] + jc_ABNM + C                 # 1 + 4 + 1 = 6 values
    if len(jc_params) != len(Pet_Cutting_Force_list):
        raise ValueError("Pet_Cutting_Force_list must match # of JC parameters")

    sensitivities = [Cutting_Force]               # Sensitivity #1

    # ----- 2) loop parameter-by-parameter --------------------------
    for p_val, Pet_h in zip(jc_params, Pet_Cutting_Force_list):
        dX = eps * p_val if p_val != 0 else eps    # avoid Δp = 0
        dh_dp = (Pet_h - Cutting_Force) / dX
...
def main():
    ...
    Cutting_Force = 622.113895

    # One Pet-Cutting_Force per parameter (TQ, A, B, N, M, C)
    Pet_Cutting_Force = [ 596.779745, 646.594943, 649.825368, 620.548149, 664.136646, 628.925968]
    ...
    with open(r"C:\Users\ougbine\Desktop\CForce\sensitivity_results.json", "w") as f:
