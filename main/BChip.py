#!/usr/bin/env python3
"""
BChip.py — multiply the SECOND Johnson-Cook plastic parameter by 1.5
           and launch an Abaqus job.

IN  : S-3193_SDIN_modified.inp   (must be in the same folder)
OUT : BChipInp.inp               (modified copy)
       BChip.odb                 (created by Abaqus)
...
OUTPUT_INP   = "BChipInp.inp"
JOB_NAME     = "BChipInp"
SCALE_FACTOR = 1.2                 # ← put your desired multiplier here
# -----------------------------------------------------------------------------


def modify_second_plastic_value(inp: str, out: str, factor: float = 1.0) -> None:
    """
    Find '*Plastic, hardening=JOHNSON COOK', and in the very next line
    multiply the SECOND comma-separated value by `factor`.
    """
    ...
    while i < len(lines):
        ...
        if line.strip().lower().startswith("*plastic") and "johnson cook" in line.lower():
            ...
            if len(parts) < 2 or parts[1] == "":
                raise ValueError("Could not parse SECOND Johnson-Cook parameter.")
            ...
            parts[1] = fmt.format(orig_val * factor)
...
if __name__ == "__main__":
    ...
    print(f"Modifying SECOND Johnson-Cook value in '{inp_in}' ×{SCALE_FACTOR} …")
    modify_second_plastic_value(inp_in, inp_out, factor=SCALE_FACTOR)
