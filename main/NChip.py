#!/usr/bin/env python3
"""
NChip.py — multiply the THIRD Johnson-Cook plastic parameter by 1.5
           and launch an Abaqus job.

IN  : S-3193_SDIN_modified.inp   (must reside beside this script)
OUT : NChipInp.inp               (modified copy)
       NChip.odb                 (created by Abaqus)
"""
...
OUTPUT_INP   = "NChipInp.inp"
JOB_NAME     = "NChipInp"
SCALE_FACTOR = 1.2                  # ← put your desired multiplier here
# -----------------------------------------------------------------------------


def modify_third_plastic_value(inp: str, out: str, factor: float = 1.0) -> None:
    """
    Locate '*Plastic, hardening=JOHNSON COOK' and in the next line
    multiply the THIRD comma-separated value by `factor`.
    """
    ...
    while i < len(lines):
        ...
        if line.strip().lower().startswith("*plastic") and "johnson cook" in line.lower():
            ...
            if len(parts) < 3 or parts[2] == "":
                raise ValueError("Could not parse THIRD Johnson-Cook parameter.")
            ...
            parts[2] = fmt.format(orig_val * factor)
...
if __name__ == "__main__":
    ...
    print(f"Modifying THIRD Johnson-Cook value in '{inp_in}' ×{SCALE_FACTOR} …")
    modify_third_plastic_value(inp_in, inp_out, factor=SCALE_FACTOR)
